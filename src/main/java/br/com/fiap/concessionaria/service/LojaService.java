...
public class LojaService implements ServiceDTO<Loja, LojaRequest, LojaResponse> {
    ...
    public LojaResponse toResponse(Loja e) {
        return LojaResponse.builder()
                .id(e.getId())
                .nome(e.getNome())
                .veiculosComercializados(Optional.ofNullable(e.getVeiculosComercializados())
                        .map(veiculos -> veiculos.stream().map(veiculoService::toResponse).collect(Collectors.toSet()))
